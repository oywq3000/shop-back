...
public class SliderImageUtil {
    ...
    public static Map<String, Object> pictureTemplatesCut(
        ...
        //新建一个和模板一样大小的图像，TYPE_4BYTE_ABGR 表示具有8位RGBA颜色分量的图像，正常取imageTemplate.getType()
        BufferedImage newImage = new BufferedImage(sliderWidth,sliderHeight,sliderImage.getType());
        //得到笔画对象
        Graphics2D graphics = newImage.createGraphics();
        //如果要生成RGB格式，需要做如下配置，Transparency 设置透明
        graphics.getDeviceConfiguration().createCompatibleImage(sliderWidth,sliderHeight,Transparency.TRANSLUCENT);
        //新建的图像根据模板颜色赋值,源图生成遮罩
        ImageUtil.cutByTemplate(originalImage,sliderImage,newImage,randomX,randomY);

        //干扰项
        if(interfereNum>0){
            BufferedImage interfereSliderImage =
                    ImageIO.read(Base64DecodeMultipartFile
                            .base64ToInputStream(interfereSliderFile.getBase64()));
        ...
        graphics.dispose();
        //添加水印
        // ImageUtil.addWatermark(originalImage, watermark); todo
        //新建流
        ByteArrayOutputStream newImageOs = new ByteArrayOutputStream();
        //利用ImageIO类提供的write方法，降bi以jpg图片的数据模式写入流
        ...
        //利用ImageIO类提供的write方法，将bi以jpg图片的数据模式写入
